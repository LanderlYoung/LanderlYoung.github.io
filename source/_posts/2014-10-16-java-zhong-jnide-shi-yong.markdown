...
最近来电项目打算加入变声的功能，所以决定实用SoundTouch（这个库是LGPL协议的，所以对SoundTouch的代码没有修改的话是不需要开源代码的！LOL~~）。SoundTouch是用C++写的，因此需要一个Java的JNI封装，以便JAVA层比较方便的调用。这次用JNI封装SoundTouch的经历，还是颇有收获的。记录一下，方便以后查看。
...
Java通过JNI调用本地方法的过程大致是
 ...
 2. 通过`javah`命令生成java类对应的C/C++头文件。`javah -encoding utf-8 -cp src com.tencent.soundtouch.SoundTouch`  
 3. 在C/C++中实现头文件中声明的函数  
...
#include <jni.h>
/* Header for class com_tencent_soundtouch_SoundTouch */

#ifndef _Included_com_tencent_soundtouch_SoundTouch
#define _Included_com_tencent_soundtouch_SoundTouch
#ifdef __cplusplus
extern "C" {
#endif
#undef com_tencent_soundtouch_SoundTouch_SETTING_USE_AA_FILTER
#define com_tencent_soundtouch_SoundTouch_SETTING_USE_AA_FILTER 0L
	/*
...
文件开头就是普通的头文件，但是可以发现： 
 ...
 4. 方法的声明显得有点奇怪，由以下及部分组成：   
	 ...
	 4.  Java_com_tencent_soundtouch_SoundTouch_native_1getDefaultSampleElementSize —— 超级长的函数名！！！格式是`Java_` + `类全名` + `_` + JAVA中声明的native方法名。其中会把包名中的点（`.`）替换成下划线（`_`），同时为了避免冲突把下划线替换成`_1`。  
...
```cpp
//修改包名时只需要改以下的宏定义即可
#define FULL_CLASS_NAME "com/tencent/soundtouch/SoundTouch"
#define func(name) Java_ ## com_tencent_soundtouch_SoundTouch_ ## name
#define constance(cons) com_tencent_soundtouch_SoundTouch_ ## cons
```
比如`func(native_1newInstance)`展开成：`Java_com_tencent_soundtouch_SoundTouch_native_1newInstance`即JNI中需要导出的函数名（不过用动态注册方式没太大用了）

`constance(AUDIO_FORMAT_PCM16)`展开成`com_tencent_soundtouch_SoundTouch_AUDIO_FORMAT_PCM16`这个着实有用。
