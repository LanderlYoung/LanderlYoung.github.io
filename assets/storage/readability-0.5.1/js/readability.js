...
function grabArticle() {
  ...
  if(topDiv == null)
    ...
    /*
       ...
       <a href='http://www.arc90.com'><img src='http://www.arc90.com/images/favicon.png'></a>\
       <div class='footer-right' >\
       <span class='version'>Powered by <a href='http://https://code.google.com/p/arc90labs-readability/'>Readability version " + readabilityVersion + "</a></span>\
       </div>\
       ";
       */
...
function cleanCssAndScripts(e) {
  e = e || document;
  var scripts = e.getElementsByTagName("script");
  var max_limit = 512;

  function noMatch(str, pattern) {
    return typeof str != 'string' || !str.match(pattern);
  }

  var len = 0;
  var count = 0;//avoid dead loop
  while(scripts.length > len && count < max_limit) {
    var script = scripts[len];
    if(noMatch(script.src, /readability/i)) {
  ...
  len = 0;
  count = 0;
  while (styles.length > len && count < max_limit) {
    var style = styles[len];
    if (noMatch(style.href, /readability/i)) {
  ...
  while (styles.length > len && count < max_limit) {
    var style = styles[len];
    if(noMatch(style.href, /readability/i) &&
       typeof style.rel == 'string' &&
         style.rel.match(/stylesheet|css/i)) {
      style.parentNode.removeChild(style);
    } else {
      len++;
    }
